...
if (!blobToken.startsWith("vercel_blob_rw_")) {
  throw new Error(
    'Invalid YRWQ_READ_WRITE_TOKEN format. Token should start with "vercel_blob_rw_"',
  );
}
      ...
      if (!imageResponse.ok) {
        if (imageResponse.status === 403) {
          return NextResponse.json(
            {
              error:
                "Failed to fetch image from external service: Access Denied",
              url: null,
            },
        ...
        throw new Error(
          `Failed to fetch image: ${imageResponse.statusText}. Status: ${imageResponse.status}`,
        );
      }
          ...
          return NextResponse.json(
            {
              ...
              details:
                "Please check your YRWQ_READ_WRITE_TOKEN and ensure it has the correct permissions",
            },
            { status: 401 },
          );
        }
